...
function OpenMenuGarage(garage, KindOfVehicle)
	...
	local elements = {
		{label = "Veículo de retorno ("..Config.Price.."$)", value = 'return_vehicle'},
	}
...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if(v.fourrieremecano)then
					labelvehicle = vehicleName..': Patio do mecanico'
				elseif (v.state)  then
					labelvehicle = vehicleName..': voltou'
				else
		...
		function(data, menu)
			...
			table.insert(elem, {label ="Renomeie o carro" , value = 'rename_vehicle'})
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Seu veículo está na libra mecânica')
					...
					elseif data2.current.value == "rename_vehicle" then
						AddTextEntry('FMMC_KEY_TIP8', "Nome do veículo desejado")
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		TriggerEvent('esx:showNotification', 'Não há veículo para retornar')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'O trailer está de volta na libra')
					else
						TriggerEvent('esx:showNotification', 'Você não pode armazenar este trailer no promontório')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'O veículo está de volta na libra')
					else
						TriggerEvent('esx:showNotification', 'Você não pode armazenar este veículo no promontório')
					end
				end,vehicleProps)
			end
		else
...
function ReturnVehicleMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		function(data, menu)
			if data.current.value == 'fourrieremecano' then
				ESX.ShowNotification("Verifique com a polícia ou mecano para descobrir como recuperar seu carro.")
