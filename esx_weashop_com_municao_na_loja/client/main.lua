...
function OpenBuyLicenseMenu (zone)
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'shop_license',
    {
      ...
      elements = {
        { label = _U('yes') .. ' (R$' .. Config.LicensePrice .. ')', value = 'yes' },
...
function OpenShopMenu(zone)
  ...
  for i=1, #Config.Zones[zone].Items, 1 do
    ...
    table.insert(elements, {
      label     = item.label .. ' - <span style="color:green;">R$' .. item.price .. ' </span>',
...
-- thx para Pandorina pelo script
...
AddEventHandler('esx_weashop:clipcli', function()
  ...
  if IsPedArmed(ped, 4) then
    ...
    if hash~=nil then
      ...
      ESX.ShowNotification(_U('did_you_use_a_charger'))
    else
      ESX.ShowNotification(_U('you_do_not_have_a_gun_in_your_hand'))
    end
  else
    ESX.ShowNotification(_U('this_type_of_ammunition_is_not_suitable'))
  end
end)
